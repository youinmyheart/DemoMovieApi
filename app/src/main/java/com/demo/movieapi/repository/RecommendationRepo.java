...
public class RecommendationRepo {
    private static final String TAG = RecommendationRepo.class.getSimpleName();

    public MutableLiveData<DataWrapper<TMDBResponse>> getMovieRecommendation(int movieId, int page) {
        final MutableLiveData<DataWrapper<TMDBResponse>> liveData = new MutableLiveData<>();
        Call<TMDBResponse> call = APIManager.getMovieRecommendations(movieId, page);
